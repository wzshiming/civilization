...
Consumable {
    id: ConsumableID
    type: ConsumableType
    resourceType: ResourceTypeID?    // Set if type is RESOURCE
    speciesType: SpeciesTypeID?      // Set if type is SPECIES_POPULATION
}

ConsumptionRule {
    consumer: SpeciesTypeID          // Species that consumes
    consumables: []ConsumableRef     // What can be consumed
    efficiency: float                // Conversion efficiency (0.0 to 1.0)
}
