...
Uses Earth's natural calendar system.

```
daysPerYear: int = 365
daysPerMonth: int = 30      // Approximate (12 months × 30 = 360 days, with 5 extra days)
...
Species consume resources to satisfy various needs (food, tools, luxury goods, etc.).
Consumption affects species mood, reproduction rate, and overall well-being.

```
NeedType: FOOD, TOOLS, SHELTER, LUXURY, MEDICINE, EDUCATION, etc.

ConsumptionNeed {
    needType: NeedType               // Type of need
    requiredAmount: float            // Amount needed per capita per day
    satisfactionThreshold: float     // Minimum satisfaction level (0.0 to 1.0)
    impactOnMood: float             // How much this affects mood (-1.0 to 1.0)
    impactOnReproduction: float     // How much this affects reproduction rate
}

ResourceAttribute {
    needType: NeedType              // What need this resource satisfies
    effectivenessValue: float       // How effectively it satisfies the need (0.0 to 1.0)
}
```
...
Species {
    ...
    needs: []ConsumptionNeed         // What this species needs to consume (food, tools, etc.)
}
...
Resources are produced by plots and buildings, and consumed by species to satisfy needs.
...
ResourceType {
    ...
    attributes: []ResourceAttribute  // What needs this resource satisfies (food, tools, etc.)
...
- Impact ranges: -1.0 (very negative) to 1.0 (very positive)
- Resources can satisfy multiple needs with different effectiveness values
