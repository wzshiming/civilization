...
### Building and Population Types

```
Building {
    type: BuildingType          // FARM, MINE, FACTORY, TEMPLE, etc.
    level: int                  // Building upgrade level
    workers: []WorkerAssignment // Workers assigned to this building
    resourceModifiers: []ResourceModifier  // Resource changes caused by this building
}

WorkerAssignment {
    populationID: PopulationID  // Reference to population working here
    efficiency: float           // Work efficiency (0.0 to 1.0)
}

ResourceModifier {
    resourceType: ResourceType  // Which resource is affected
    quantityCapacity: float     // Additional storage/upper limit for resource
    dailyChangeModifier: float  // Change to daily production/consumption rate
    monthlyChangeModifier: float // Change to monthly production/consumption rate
}

Population {
    id: PopulationID
    type: SpeciesType           // HUMAN, ELF, DWARF, etc.
    count: int                  // Number of individuals
    growth: float               // Population growth rate
    happiness: float            // Happiness level (0.0 to 1.0)
    culture: CultureID          // Cultural identity
    religion: ReligionID        // Religious affiliation
    employedIn: BuildingID?     // Building where this population works (null if unemployed)
    resourceEffects: []ResourceEffect  // Resource changes from population existence
    diffusion: DiffusionInfo    // Migration/spread to neighboring plots
    growthModifiers: []GrowthModifier  // Population change factors based on local conditions
}

ResourceEffect {
    resourceType: ResourceType  // Which resource is affected
    dailyConsumption: float     // Daily consumption per capita
    dailyProduction: float      // Daily production per capita (if applicable)
    monthlyConsumption: float   // Monthly consumption per capita (derived: dailyConsumption * daysPerMonth)
    monthlyProduction: float    // Monthly production per capita (derived: dailyProduction * daysPerMonth)
}
// Note: daysPerMonth is a game calendar constant (typically 30)
// Note: Monthly values should be recalculated when daily values change to maintain consistency

DiffusionInfo {
    migrationRate: float        // Max % of population that can migrate out per month (0.0 to 1.0)
    migrationPreference: []TerrainType  // Preferred terrain types for migration
    spreadRadius: int           // How many plots away population can spread
}
// Note: Actual migration depends on destination capacity, happiness, and other factors

GrowthModifier {
    factor: GrowthFactorType    // FOOD, TEMPERATURE, HAPPINESS, OVERCROWDING, etc.
    currentValue: float         // Current value of the factor
    impact: float               // Impact on growth rate (-1.0 to 1.0)
}

// Note: Building.workers is the source of truth for employment. Population.employedIn is derived.
// Note: Religion and Culture types are defined in 000.definition.md
```

**Discovery State**: Players are added to discoveredBy arrays when they explore plots. State persists across game sessions.
