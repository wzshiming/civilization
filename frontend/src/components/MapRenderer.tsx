...
export function MapRenderer({ worldMap, onParcelClick }: MapRendererProps) {
  ...
  const updateContainer = useCallback(
    (container: Container, x: number, y: number, scale: number) => {
      container.x = x
      container.y = y
  ...
  const updateCameraLoop = useCallback(() => {
    ...
    const moveX =
      (keysRef.current.has('d') || keysRef.current.has('D') || keysRef.current.has('ArrowRight')
        ? -MOVE_SPEED
        : 0) +
      (keysRef.current.has('a') || keysRef.current.has('A') || keysRef.current.has('ArrowLeft')
        ? MOVE_SPEED
        : 0)
    const moveY =
      (keysRef.current.has('s') || keysRef.current.has('S') || keysRef.current.has('ArrowDown')
        ? -MOVE_SPEED
        : 0) +
      (keysRef.current.has('w') || keysRef.current.has('W') || keysRef.current.has('ArrowUp')
        ? MOVE_SPEED
        : 0)
    ...
    const isZooming =
      zoomPointRef.current !== null && Math.abs(zoomRef.current - oldZoom) > ZOOM_CHANGE_THRESHOLD
