...
Plots store dynamic attributes that can be extended without schema changes:

- **Terrain Attributes**: Altitude, temperature, humidity, fertility
- **Resources**: Stored as key-value pairs (e.g., `"resources.iron": 50`)
- **Versioning**: Each attribute includes `lastUpdated` timestamp and `version` for tracking changes
- **Extensibility**: New attributes can be added dynamically during gameplay

Attributes are accessed via `plot.plotAttributes.attributes[attributeName]` and follow the `Attribute` structure defined in [000.definition.md](000.definition.md).
